...
if [ -z "$UPGRADE_KAFKA_STREAMS_TEST_VERSION" ]; then
  for dir in "$base_dir"/core/build/dependant-libs-${r"${SCALA_VERSION}"}*;
  do
...
if [ -z "$UPGRADE_KAFKA_STREAMS_TEST_VERSION" ]; then
  ...
  rocksdb_lib_dir=$(dirname $0)/../streams/build/dependant-libs-${r"${SCALA_VERSION}"}
else
...
if [ -z "$UPGRADE_KAFKA_STREAMS_TEST_VERSION" ]; then
  ...
  SHORT_VERSION_NO_DOTS=${r"${VERSION_NO_DOTS:0:((${#VERSION_NO_DOTS} - 1))}"} # remove last char, ie, bug-fix number
...
for dir in "$base_dir"/tools/build/dependant-libs-${r"${SCALA_VERSION}"}*;
do
  CLASSPATH="$CLASSPATH:$dir/*"
done
...
do
  for file in "$base_dir"/connect/${r"${cc_pkg}"}/build/libs/connect-${r"${cc_pkg}"}*.jar;
  do
    if should_include_file "$file"; then
      CLASSPATH="$CLASSPATH":"$file"
    fi
  done
  if [ -d "$base_dir/connect/${r"${cc_pkg}"}/build/dependant-libs" ] ; then
    CLASSPATH="$CLASSPATH:$base_dir/connect/${r"${cc_pkg}"}/build/dependant-libs/*"
  fi
done
...
for file in "$base_dir"/core/build/libs/kafka_${r"${SCALA_BINARY_VERSION}"}*.jar;
do
  if should_include_file "$file"; then
    CLASSPATH="$CLASSPATH":"$file"
  fi
...
if [ -z "$KAFKA_LOG4J_OPTS" ]; then
  ...
  (( CYGWIN )) && LOG4J_DIR=$(cygpath --path --mixed "${r"${LOG4J_DIR}"}")
  KAFKA_LOG4J_OPTS="-Dlog4j.configuration=file:${r"${LOG4J_DIR}"}"
else
...
if [ "x$KAFKA_DEBUG" != "x" ]; then
    ...
    DEFAULT_JAVA_DEBUG_OPTS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=${r"${DEBUG_SUSPEND_FLAG:-n}"},address=$JAVA_DEBUG_PORT"
...
(( CYGWIN )) && CLASSPATH=$(cygpath --path --mixed "${r"${CLASSPATH}"}")
